...
class Chat extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->userdata['role_id'] == 2) {
            ...
            $data['strTitle'] = 'Daftar Dokter';
            $data['strsubTitle'] = 'Dokter';
            $data['chatTitle'] = 'Pilih dokter dibawah';
        } else {
            $list = $this->UserModel->ClientsListCs();
            $data['strTitle'] = 'Daftar Pelanggan';
            $data['strsubTitle'] = 'Pelanggan';
            $data['chatTitle'] = 'Pilih pelanggan untuk memulai chat';
        }
        $vendorslist = [];
        foreach ($list as $u) {
            $vendorslist[] =
        ...
        $this->load->view('pages/user/diagnosa/tanya-dokter', $data); // 
